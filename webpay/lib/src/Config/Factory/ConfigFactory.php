...
class ConfigFactory implements ConfigFactoryInterface {
    ...
    const DEPOSIT_FLAG = 'depositFlag';
    const PRIVATE_KEY_PASSPHRASE = 'privateKeyPassphrase';
    const PUBLIC_KEY = 'publicKey';
    const RESPONSE_URL = 'responseUrl';

    /**
     * @var \Pixidos\GPWebPay\Config\Factory\PaymentConfigFactory
     */
     ...
     */
    public function create($params, $defaultGateway = 'default') {
     ...
     */
    private function normalizeParams($params, $defaultGateway) {
     ...
     */
    private function getValue($key, $data, $gateway) {
     ...
     * @return Config
     */
    private function createConfig($defaultGateway) {
     ...
     * @param Config                           $config
     */
    private function processParams($params, Config $config) {
        ...
        foreach ($params as $gateway => $data) {
            $paymentConfig->addPaymentConfig(
                $this->paymentConfigFactory->create(
                    ...
                    $gateway,
                    (string) ($data[self::RESPONSE_URL] ? $data[self::RESPONSE_URL] : null)
                )
            );
