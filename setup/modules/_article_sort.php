...
if (is_array($_POST['items'])) {
    ...
    foreach ($_POST['items'] as $key => $val) {
        $queryString = "update " . TABLE_PREFIX . "article set sorter = '" . $i . "' where 1 and article_id = '" . $val . "';";
                ...
                <tr>
                    <td rowspan="2">
                        <select name="items[]" size="10" multiple="multiple" id="items[]" style="width: 98%;">
                            <?php
                            $queryString = "select article_id as id, sk_name as name from " . TABLE_PREFIX . "article where 1 and article_category_id = '" . $_GET['article_category_id'] . "' order by sorter;";
                            if (!$Result = mysql_query($queryString)) {
