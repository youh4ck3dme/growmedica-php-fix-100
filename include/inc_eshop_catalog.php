...
<div id="catalogue" class="carousel<?= (count($products) == 0 ? ' empty' : ''); ?>">
                        ...
                        if (count($products) != 0) {
                            foreach ($products as $rowC) {
                                $query_eshop_category = 'SELECT ' . $_SESSION['lang'] . '_name_seo FROM ' . TABLE_PREFIX . 'menu AS m
                                    JOIN ' . TABLE_PREFIX . 'product_menu AS pm USING (menu_id) 
                                    WHERE pm.product_id = ' . $rowC->product_id . '
                                    LIMIT 1;';
                                $result_eshop_category = mysql_query($query_eshop_category);
                                $eshop_category = mysql_fetch_row($result_eshop_category) ;
                                ?>
                                <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 no-gutter">
                                    <!--<div class="left-bar">
                                        <? 
                                        if ($rowC->action == 1) {
                                    ...
                                    <div class="item">
                                        <a href="<?= ROOTDIR . '/' . !empty($eshop_category[0]) ? $_SESSION['lang'] . '/' . $eshop_category[0] : Menu::getHyperLinkById(ESHOP_MAIN_CATEGORY) ?>/produkt/<?= String::SEOFriendlyText($rowC->name) . '/' . $rowC->product_id ?>" title="<?= $rowC->name ?>">
                                            <div class="crust ratio-4_3">
                                                <div class="core">
                                                    <?
                                                    if (is_file("./photos/thumbnail/" . $rowC->image_src)) { ?>
                                                        <img src="photos/thumbnail/<?= $rowC->image_src; ?>" alt="<?= $rowC->name; ?>" width="300" height="400" loading="lazy" /><?
                                                    } else {
                                                        echo '<center>' . $cTranslator->getTranslation('Žiaden obrázok', 0) . '</center>';
                                                        echo '<img src="images/wrapper/no-product-photo.png" alt="" width="300" height="400" loading="lazy" />';
                                                    }
                                            ...
                                            <div class="directive">
                                                ...
                                                <div class="price-box">
                                                    <?
                                                    if($rowC->delivery_time == '1') {
                                                        echo '<img src="images/wrapper/skladom.svg" alt="" width="12" height="12" loading="lazy"><span style="font-size:9px;color:#4A6D22;">'.$cTranslator->getTranslation('skladom', 0).'</span>';
                                                    }
                                                    else {
                                                        echo '<img src="images/wrapper/na_objednavku.svg" alt="" width="12" height="12" loading="lazy"><span style="font-size:9px;color:#936544;">'.$cTranslator->getTranslation('na objednávku', 0).'</span>';
                                                    }
