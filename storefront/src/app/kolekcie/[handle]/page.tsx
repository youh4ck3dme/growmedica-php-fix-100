      ...
      <Container>
        ...
        <nav aria-label="Breadcrumb" className="mb-6">
          <ol className="flex items-center gap-2 text-sm text-(--color-text-muted)">
            ...
            <li aria-hidden="true">/</li>
            <li className="text-(--color-text) font-medium" aria-current="page">{c.title}</li>
          </ol>
        </nav>

        {/* Header */}
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-(--color-text) mb-2">{c.title}</h1>
          {c.description && (
            <p className="text-(--color-text-muted) max-w-2xl">{c.description}</p>
          )}
        </header>
